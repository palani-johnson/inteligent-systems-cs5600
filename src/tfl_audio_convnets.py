...
base_path = "/home/palani/Projects/School/inteligent-systems-cs5600/data/project01/"
BUZZ1_base_path = base_path + "BUZZ1/"
BUZZ2_base_path = base_path + "BUZZ2/"
BUZZ3_base_path = base_path + "BUZZ3/"
CONVNET_PATH = "data/nets_project01/aud_cn/aud_cn.tfl"
...
def __conv_template():
    ...
    net = conv_2d(net, nb_filter=32, filter_size=25, activation="relu")
...
def train_tfl_audio_convnet_model(
...
if __name__ == "__main__":
    aud_cn = (
        make_audio_convnet_model()
        if input("New?[y/n] ").upper() == "Y"
        else load_audio_convnet_model(CONVNET_PATH)
    )

    train_tfl_audio_convnet_model(
        aud_cn,
        np.concatenate(
    ...
    print(
        "BUZZ1 acc: {}".format(
            validate_tfl_audio_convnet_model(
                aud_cn,
                BUZZ1_valid_X,
                BUZZ1_valid_Y,
            )
